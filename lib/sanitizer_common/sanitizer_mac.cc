...
# if defined(__aarch64__)
  *pc = ucontext->uc_mcontext->STATE_MEM.THREAD_MEM(pc);
#   if defined(__IPHONE_8_0) && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_8_0
  *bp = ucontext->uc_mcontext->STATE_MEM.THREAD_MEM(fp);
#   else
  *bp = ucontext->uc_mcontext->STATE_MEM.THREAD_MEM(lr);
#   endif
  *sp = ucontext->uc_mcontext->STATE_MEM.THREAD_MEM(sp);
...
# elif defined(__arm__)
  *pc = ucontext->uc_mcontext->STATE_MEM.THREAD_MEM(pc);
  *bp = ucontext->uc_mcontext->STATE_MEM.THREAD_MEM(r[7]);
  *sp = ucontext->uc_mcontext->STATE_MEM.THREAD_MEM(sp);
